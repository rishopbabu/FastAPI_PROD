...
from .. import models, schemas, utils, oauth2 # to use fast api methods import other methods
...
def create_user(user: schemas.CreateUser, db: Session = Depends(get_db)):
    ...
    hashed_pwd = utils.hash_password(user.password) # getting the password from user input and store in a variable
...
# Get all users
@router.get('/get_all_users', status_code=status.HTTP_200_OK, response_model=List[schemas.GetUserResponse])
...
@router.get("/get_user/{id}", name="Get users by Id", response_model=schemas.GetUserResponse)
